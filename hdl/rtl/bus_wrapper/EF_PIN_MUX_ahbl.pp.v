...
module EF_PIN_MUX_ahbl ( // The module is not parametrized with the number of IOs because registers for the function select need to be added according to the number of IOs
	...
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) FN_SEL1_REG <= 0; else if(ahbl_we & (last_HADDR[15:0]==FN_SEL1_REG_ADDR)) FN_SEL1_REG <= HWDATA[32-1:0];
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) FN_SEL2_REG <= 0; else if(ahbl_we & (last_HADDR[15:0]==FN_SEL2_REG_ADDR)) FN_SEL2_REG <= HWDATA[32-1:0];
